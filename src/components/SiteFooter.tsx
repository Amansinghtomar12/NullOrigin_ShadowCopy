      ...
      <div className="shell py-12 grid grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="col-span-2 lg:col-span-1 space-y-3">
          <div className="flex items-center gap-2.5">
            ...
            />
            <div>
              <p className="font-display font-extrabold text-[14px] tracking-[0.12em] text-white">
                NULL ORIGIN CTF
              </p>
              <p className="font-mono text-[10px] tracking-[0.18em] text-[var(--faint)] mt-0.5">
                Team CyberXoX
              </p>
            </div>
          </div>
          <p className="text-[14px] text-[var(--muted)] leading-relaxed max-w-xs">
        ...
        <div>
          <p className="font-mono text-[10.5px] tracking-[0.2em] uppercase text-red-400 mb-3">Explore</p>
          <div className="space-y-2.5">
            {NAV.map((l) => (
              <a
                key={l.id}
                href={`#${l.id}`}
                className="block py-1.5 text-[14px] text-[var(--muted)] hover:text-white transition-colors"
              >
                {l.label}
              </a>
            ))}
        ...
        <div>
          <p className="font-mono text-[10.5px] tracking-[0.2em] uppercase text-red-400 mb-3">Partner</p>
          <a
            href={`mailto:${PARTNER_EMAIL}`}
            className="block text-[14px] text-[var(--muted)] hover:text-white transition-colors break-all"
          >
            {PARTNER_EMAIL}
          </a>
        ...
        <div className="col-span-2 lg:col-span-4 border-t border-[var(--line-soft)] pt-7 flex flex-col sm:flex-row items-center justify-center gap-x-12 gap-y-5 flex-wrap">
          {PARTNERS.map((p) => (
            ...
            >
              <span className="font-mono text-[10px] font-bold tracking-[0.2em] uppercase text-[var(--faint)]">
                {p.tier}
              </span>
