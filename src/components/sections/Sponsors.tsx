  ...
  return (
    <div className="slot slot--filled slot--feature flex-col lg:flex-row items-stretch gap-0 p-0 text-left">
      ...
      <div className="relative flex-1 p-6 sm:p-8">
        <p className="font-mono text-[11px] tracking-[0.22em] uppercase text-[var(--accent)] flex items-center gap-2">
        ...
        <p className="text-[17px] leading-relaxed text-[var(--muted)] mt-3">{about}</p>

        <p className="font-mono text-[11px] tracking-[0.22em] uppercase text-red-400 mt-6 mb-3">
          What this partnership covers
        </p>
        <ul className="grid sm:grid-cols-2 gap-x-5 gap-y-2">
          {includes.map((item) => (
            <li key={item} className="flex items-start gap-2.5 text-[17px] text-[var(--muted)]">
              ...
              {WHY_SPONSOR.map((w) => (
                <div key={w.title} className="tilt3d d3">
                  <div className="tilt3d__inner">
                    ...
                    <p className="text-[17px] text-[var(--muted)] leading-relaxed mt-1.5">{w.desc}</p>
                  </div>
                </div>
              ))}
            </div>
